...
export default function ContactsTable({ contacts, onContactClick, onEdit, onDelete, onEmail }: any) {
    ...
    <div className="overflow-x-auto">
      {selected.length > 0 && (
        <div className="bg-blue-50 border-b border-blue-200 px-6 py-3 flex justify-between items-center">
          <span className="text-sm text-blue-700">{selected.length} contacts selected</span>
          <div className="flex gap-2">
            ...
            <button className="px-3 py-1 text-sm bg-white text-gray-700 rounded-lg hover:bg-gray-50"><Phone size={14} className="inline mr-1" />Call</button>
          </div>
        </div>
      )}
      <table className="w-full">
        ...
        <tbody className="divide-y">
          {contacts.map((contact: any) => (
            <tr key={contact.id} className="hover:bg-gray-50 group cursor-pointer" onClick={() => onContactClick(contact)}>
